...
var start = function () { return __awaiter(void 0, void 0, void 0, function () {
    var spinner, readPortfolioFx, startFetchingFx, logFx;
    return __generator(this, function (_a) {
        spinner = ora_1.default({ spinner: 'circle' });
        spinner.start();
        readPortfolioFx = effector_1.createEffect(function () { return helper_1.readLocalConfig().orders; });
        ...
        effector_1.sample({
            ...
            target: store_1.addGroupedOrders,
            fn: function (orders) { return lodash_1.groupBy(orders, 'ticker'); },
        });
        effector_1.sample({
            source: store_1.$metrics,
            clock: store_1.$groupedOrders.updates,
            target: startFetchingFx,
            fn: function (metrics, groupedOrders) { return ({ groupedOrders: groupedOrders, metrics: metrics }); },
        });
        effector_1.forward({ from: startFetchingFx.doneData, to: store_1.addTableData });
        effector_1.sample({
            source: [store_1.$summaryData, store_1.$errors],
            clock: store_1.$tableData.updates,
            target: logFx,
            fn: function (_a, tableData) {
                var summaryData = _a[0], errors = _a[1];
                return ({ tableData: tableData, summaryData: summaryData, errors: errors });
            },
        });
        // UI effects
        readPortfolioFx.pending.watch(function (pending) {
            if (pending)
                spinner.text = 'Reading portfolio json';
        });
        readPortfolioFx.fail.watch(function (_a) {
            var error = _a.error;
            spinner.fail("JSON file read error: " + error.message);
        });
        startFetchingFx.pending.watch(function (pending) {
            if (pending)
                spinner.text = 'Fetching from server';
        });
